...
if [ "$NEWSECRETS" = "1" -o ! -s /srv/secrets.conf ]; then
	...
	sed -i -e "s/sessionSecret =.*/sessionSecret = $SESSION_SECRET/" /srv/channel-server/default.conf
	sed -i -e "s/encryptionSecret =.*/encryptionSecret = $ENCRYPTION_SECRET/" /srv/channel-server/default.conf
	sed -i -e "s/serverToken =.*/serverToken = $SERVER_TOKEN/" /srv/channel-server/default.conf
	sed -i -e "s/;sharedsecret_secret =.*/sharedsecret_secret = $SHARED_SECRET/" /srv/channel-server/default.conf
	mv /srv/secrets.conf.tmp /srv/secrets.conf
fi
echo "Server secrets:"
cat /srv/secrets.conf

echo "Staring Channel Server ..."
exec /srv/channel-server/channel-server "$@"
